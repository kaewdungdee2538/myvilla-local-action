...
export class GetSlipService {
  ...
  async getSlipOutInfo(@Body() body) {
    ...
    const company_id = body.company_id;
    return await this.getSlipOutInfoFormBase(visitor_record_id, company_id);
  }

  async getSlipOutInfoFormBase(visitor_record_id: number, company_id: number) {
    let sql = `select 
        ...
        ,guardhouse_in_code
        ,guardhouse_out_code
        ,COALESCE(CAST(visitor_record_id AS NUMERIC),0) AS visitor_record_id
        ...
        ,home_info
        ,license_plate
        ,COALESCE(tvr.payment_type_id,0) AS payment_type_id
        ,mpt.payment_type_name
        ,COALESCE(CAST(parking_payment AS NUMERIC),0) AS parking_payment
        ,COALESCE(CAST(overnight_fines AS NUMERIC),0) AS overnight_fines
        ,COALESCE(CAST(losscard_fines AS NUMERIC),0) AS losscard_fines
        ,COALESCE(CAST(total_price AS NUMERIC),0) AS total_price
        ,COALESCE(CAST(customer_payment AS NUMERIC),0) AS customer_payment
        ,payment_info

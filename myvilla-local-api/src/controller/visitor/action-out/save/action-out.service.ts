...
export class ActionOutSaveService {
    ...
    async saveOut(files: any, @Body() body) {
        ...
        if (getRecordIn) {
            const getVisitorInInfo = await this.getVisitorRecordIn(getRecordIn.visitor_record_id);
            if (getVisitorInInfo) {
    ...
    async getVisitorRecordId(@Body() body) {
        ...
        let sql = `select getvs_record_card_or_slot($1,$2,$3,$4) as visitor_record_id;`
        const query = {
            text: sql
    ...
    async getVisitorRecordIn(recordin_id: string) {
        const record_id = parseInt(recordin_id);
